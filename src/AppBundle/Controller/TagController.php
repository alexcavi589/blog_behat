        ...
        if($form->isSubmitted()){
             if($form->isValid()){
                 ...
                 if($flush==null){
                     ...
                     $status = "Error al añadir etiqueta!!!";
                 }

            $status = "La etiqueta se ha creado correctamente";
             }else{
            $status = "La etiqueta no se ha creado, porque hay fallos de validación y el formulario no es valido";
            }
        $this->session->getFlashBag()->add("status",$status);
        return $this->redirectToRoute("blog_index_tag");
        }
    return $this->render("AppBundle:Tag:add.html.twig",array(
